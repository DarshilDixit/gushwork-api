...
GW.TABS.model = (function (G) {
  ...
  var PAGE = 25;
  var data = null, err = null, root = null, seq = 0, decShown = PAGE, unrShown = PAGE;
  ...
  function activate(el) { root = el; render(); G.every('model', 300000, function () { load().then(render); }); return load().then(render); }
  ...
  function decisions(d) {
    ...
    return U.panel({ id: 'mdl-dec', title: 'Every decision, with the evidence', qual: fmt(all.length) + ' ' + G.plural(all.length, 'lead'),
      body: '<p class="lnote">The quote is what the model read on their own site and decided on. If a row looks like a real prospect, the quote is the fastest way to tell a wrong scope from a wrong call.</p>' +
        U.rtable({ ns: 'mdl', rows: rows, key: function (x) { return x.session_id; }, rowName: function (x) { return x.email || 'this lead'; },
          ...
          cols: [
            ...
            { label: 'Action', html: function (x) { var a = ACT[x.action] || [x.action, 'b-neu']; return '<span class="badge ' + a[1] + '">' + esc(a[0]) + '</span>'; } },
          ...
          detail: function (x) {
            return U.kv([['Domain judged', x.domain_judged ? '<code>' + esc(x.domain_judged) + '</code>' : null, 'html'], ['Why', x.reason], ['Quote', x.evidence_quote ? '“' + x.evidence_quote + '”' : null], ['Company', x.company], ['Their website', x.website], ['Product', x.product],
  ...
  function scrape(d) {
    ...
    var list = U.rtable({ ns: 'mdlu', rows: rows, key: function (u) { return u.domain + '|' + (u.email || ''); }, rowName: function (u) { return u.domain; },
      ...
      ] }) + more(allU.length, unrShown, 'mdl-unr');
    return U.panel({ id: 'mdl-scrape', title: 'What we could not read', qual: 'companies behind this window’s leads',
      body: '<div class="sumgrid four">' +
        ...
        U.metricCard({ id: 'mdl-deploy', label: 'Classified since this deploy', value: ip.ok, sub: fmt(ip.errored) + ' API errors · ' + fmt(ip.unreachable) + ' unreadable · cache hits ' + (ip.cacheHitPct === null || ip.cacheHitPct === undefined ? '—' : ip.cacheHitPct + '%') }) + '</div>' +
  ...
  function render() {
    ...
    var d = data;
    var head = '<section class="ph"><div class="ph-top"><h1 class="title" tabindex="-1">Model</h1><span class="readat">' + (d ? 'Read ' + esc(G.etTime(d.generatedAt)) + ' ET' : '') + '</span></div>' +
      /* TRUE FOR EVERY ROW: a late verdict marks a lead after it booked, and a
         name-only verdict never read the site */
      '<p class="lede">What the website classifier — the model — decided, and what it could not read. A <b>blocked</b> lead was stopped as real estate or insurance: before the calendar, or marked after booking when the answer came late. A <b>Meta withheld</b> lead booked, went to Salesforce and is on the SDR list as normal; only the conversion events were held back.</p>' +
  ...
  if (typeof document !== 'undefined' && document.addEventListener) document.addEventListener('click', function (e) {
    ...
    if (t.getAttribute('data-more') === 'mdl-dec') { decShown += PAGE; render(); }
    if (t.getAttribute('data-more') === 'mdl-unr') { unrShown += PAGE; render(); }
  });
  if (typeof document !== 'undefined' && document.addEventListener) document.addEventListener('change', function (e) {
    ...
    decShown = unrShown = PAGE; G.writeHash(); render(); load().then(render);
  });
