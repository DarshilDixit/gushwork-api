/* ==========================================================
  ...
  v5.16.0 — the calendar waits when an answer is seconds away. Two
    ...
    behind a calendar skeleton, capped and failing open.
  ...
  v5.13.0 — "What do you need?" on /demo, Lead Gen and AI-CRM.
    ...
    exception exactly as being on /ai-demo does -- the exception is
    about the product and the page was only ever a proxy for it -- and
    routes the booking to the CRM team via data-rh-router-crm, which
    lives in Webflow like the existing router id. The about-business
    field renders on an AI-CRM tick and is CLEARED on untick, element
    and state both, so no text is stored for a product nobody asked for.
    /demo ONLY: no checkboxes in the markup means no selection, the
    server resolves from the page as before, and product_interest stays
    NULL -- "we never asked". /ai-demo is untouched.
  ...
  v5.10.0 — NON-ICP BLOCK (real estate + insurance brand domains).
    ...
    domain or website is a national brokerage or carrier brand is sent
    to /thank-you instead of the calendar, and none of the three Meta
    events fire for them. SECTION 4C.
    The LIST LIVES ON THE SERVER, not in this file -- see the note in
    4C for why a fourth duplicated list was not acceptable here.
    Fails open everywhere: a timeout, a non-200 or no backend all mean
    "not blocked" and the lead carries on to the calendar.
