...
| `tools/fire-non-icp-slack.js` | Fires the non-ICP Slack paths for real: `blocked`, `llm-blocked`, `llm-meta`, `late-block`, `booking`, `dropoff-digest`. Lifts them out of `index.js` like `fire-alert.js`. `late-block` is the sweep's post and was fired by hand on 23 Sept — a stamped lead is NOT in Salesforce and NOT on the SDR list, so that post is the only thing telling a human a booked meeting turned out non-ICP. **`dropoff-digest` is the only fire here that reads a DATABASE** — it runs the real weekly digest over the real leads table, so on a developer machine it needs the Postgres service's `DATABASE_PUBLIC_URL`, not `gushwork-api`'s own private-network `DATABASE_URL`. Fired 25 Sept, Slack 200. It appends ONE test-marker block AFTER the digest is composed rather than passing a marker in, because the message has to stay byte-identical to what Monday sends. **`liftDecl` could not take `DROPOFF_STAGES`** — it brace-matches one declaration and truncated the array at its first object, so the dropoff block is lifted by region with a `between()` helper instead |
